...
async function hatch(initExtras) {
  ...
  const bundleParamMustBe = linCtx.parentBundle.makeParamPopper().mustBe;
  ...
  const res = this;
  const mustFact = mustBe.tProp(res.typeName + ' props',
  ...
  mustFact('nonEmpty ary', 'urls').map(rewriteUrlProtos).forEach((url) => {
